...
public class CreateMergeRequestService {
    ...
    public MergeRequest createMergeRequest(MergeToTargetBranchDTO mergeDTO) throws GitLabApiException {
        ...
        checkGitLabApiCompatibility(gitLabApi, gitlabUrl);
        ...
        logger.info("创建合并请求: GitLab URL={}, 项目ID={}, 项目路径={}, 源分支={}, 目标分支={}",
                gitlabUrl, projectId, projectPath, sourceBranch, targetBranch);

        // 首先尝试使用项目ID，如果失败（404），再尝试使用项目路径
        try {
            ...
            // 如果是404错误，尝试使用项目路径
            if (httpStatus == 404 && projectPath != null && !projectPath.isEmpty()) {
                logger.warn("使用项目ID创建合并请求失败（404），尝试使用项目路径: 项目ID={}, 项目路径={}", projectId, projectPath);
                try {
                    MergeRequest mergeRequest = gitLabApi.getMergeRequestApi().createMergeRequest(projectPath, mergeRequestParams);
                    logger.info("合并请求创建成功（使用项目路径）: MR IID={}, URL={}", mergeRequest.getIid(), mergeRequest.getWebUrl());
                    return mergeRequest;
                } catch (GitLabApiException e2) {
                    logger.error("使用项目路径创建合并请求也失败: HTTP状态码={}, 项目路径={}, 错误信息={}",
                            e2.getHttpStatus(), projectPath, e2.getMessage());
                    // 继续抛出原始错误
                }
            }

            logger.error("GitLab API调用失败: HTTP状态码={}, 项目ID={}, GitLab URL={}, 错误信息={}",
                    httpStatus, projectId, gitlabUrl, errorMessage);

            // 根据 HTTP 状态码提供更具体的错误信息
            if (httpStatus == 404) {
                throw new GitLabApiException("创建合并请求失败: 项目不存在或无权访问（404）。" +
                        "已尝试使用项目ID=" + projectId + (projectPath != null ? " 和项目路径=" + projectPath : "") + "，均失败。" +
                        "可能的原因：1) 项目ID或项目路径不正确；" +
                        "2) 源分支或目标分支不存在；" +
    ...
    private String parseProjectPath(String gitlabUrl) {
        ...
        String urlWithoutGit = gitlabUrl.replace(".git", "");
        int protocolIndex = urlWithoutGit.indexOf("://");
        if (protocolIndex > 0) {
            int pathStartIndex = urlWithoutGit.indexOf("/", protocolIndex + 3);
            if (pathStartIndex > 0 && pathStartIndex < urlWithoutGit.length() - 1) {
                return urlWithoutGit.substring(pathStartIndex + 1);
            }
        }
        return null;
    }
