...
import com.king.configuration.merge.service.CreateMergeRequestService;
import com.king.configuration.merge.service.FilesPusherToBranchService;
import com.king.configuration.merge.service.GitLabUrlParseService;
 ...
 * PAT部署Service实现类
 */
@Service("listMergeServiceImpl")
public class ListMergeServiceImpl extends ServiceImpl<MergeRecordMapper, TfDeployRecord> implements IListMergeService {

    private static final Logger logger = LoggerFactory.getLogger(ListMergeServiceImpl.class);
    ...
    @Override
    public Result<String> createListMergeRecord(TfDeployRecord deployRecord) {
        Assert.notNull(deployRecord, "合并登记信息不能为空");
        String testStage = "PAT";
                // 校验必填字段
        ...
        if (!StringUtils.hasText(gitlabUrl) || "1".equals(gitlabUrl)) {
            ...
            if (!StringUtils.hasText(targetBranch)) {
                ...
                // 根据目标分支，设置测试阶段（使用包含判断，大小写不敏感）
                if(targetBranch != null){
                    String targetBranchUpper = targetBranch.toUpperCase();
                    if(targetBranchUpper.contains("UAT") || targetBranchUpper.contains("SIT")){
                        testStage = "SIT";
                    }else if(targetBranchUpper.contains("PAT")){
                        testStage = "PAT";
                    }
                }
            }
        ...
        // 调用 SaveDeployRecordService 保存合并登记信息
        ...
        logger.info("清单合并登记创建成功: versionCode={}", versionCode);

        // 返回成功结果
        return Result.success(versionCode, "合并登记创建成功");
    }
